...
4. Add the following secrets:
   ...
   - `S3_ACCESS_KEY`: Your S3 access key. You can get free 10GB S3 storage at [Cloudflare](https://developers.cloudflare.com/r2/). Create a bucket with the name `book`.
   - `S3_SECRET_KEY`: Your S3 secret key.
   - `S3_ENDPOINT`: Your S3 bucket endpoint.
5. Go to the `Variables` tab and add the following variables:
...
6. Create a local folder of the name `Chinese Book Name` and place the book file in the folder. Rename the file to `input.docx` or `input.epub`. 
7. Upload the folder to Cloudflare S3 bucket `book`. (**ATTENTION**: Keep the folder structure, don't upload the file directly to the bucket)
8. Go to the `Actions` tab and manually trigger the workflow.
9. The translated book will be available in both Chinese and bilingual (English + Chinese) formats in your S3 bucket.
